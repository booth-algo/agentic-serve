...
def write_report(rows: list[dict], summary: dict[str, dict[str, float]], path: Path) -> None:
    ...
    lines.append("```")
    lines.append("")
    lines.append("`effective_c` defaults to cohort `c` but may be overridden by the observed active count (`engine_max_decode_batch`) when available; this lets the model recover when sessions complete and exit (multi-turn benches).")
    lines.append("")
