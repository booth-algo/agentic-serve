...
def create_multi_node_interconnect(
    num_nodes: int,
    gpus_per_node: int,
    intra_node_link: str = "NVLinkV4",
    inter_node_link: str = "InfiniBandNDR",
    intra_node_links_per_gpu: int = 18,  # H100 has 18 NVLink4 links
    inter_node_links_per_gpu: int = 1,   # Typically 1 NIC per GPU
    topology: TopologyType = TopologyType.FC,
) -> "InterConnectModule":
    """
    Create a multi-node interconnect with heterogeneous intra/inter-node bandwidth.

    Uses the intra-node link as the base link module and creates a bandwidth_matrix
    that scales down inter-node pairs proportionally.

    Args:
        num_nodes: Number of nodes in the cluster
        gpus_per_node: Number of GPUs per node
        intra_node_link: Link type for intra-node communication (default NVLinkV4)
        inter_node_link: Link type for inter-node communication (default InfiniBandNDR)
        intra_node_links_per_gpu: Number of intra-node links per GPU
        inter_node_links_per_gpu: Number of inter-node links (NICs) per GPU
        topology: Network topology type

    Returns:
        InterConnectModule with proper bandwidth_matrix for multi-node
    """
    total_gpus = num_nodes * gpus_per_node
    intra_link = link_module_dict[intra_node_link]
    inter_link = link_module_dict[inter_node_link]

    # Calculate effective bandwidths
    # Intra-node: e.g., 18 NVLink4 × 50 GB/s = 900 GB/s bidi total
    intra_bw_bidi = intra_link.bandwidth_both_direction * intra_node_links_per_gpu
    # Inter-node: e.g., 1 NIC × 100 GB/s = 100 GB/s bidi
    inter_bw_bidi = inter_link.bandwidth_both_direction * inter_node_links_per_gpu

    # Bandwidth ratio: inter-node relative to intra-node
    # This gets multiplied by the base link bandwidth in get_bandwidth()
    inter_ratio = inter_bw_bidi / intra_bw_bidi

    # Build bandwidth matrix
    bandwidth_matrix = {}
    device_node_mapping = {}

    for dev_id in range(total_gpus):
        node_id = dev_id // gpus_per_node
        device_node_mapping[dev_id] = node_id
        bandwidth_matrix[dev_id] = {}

        for other_dev in range(total_gpus):
            if dev_id == other_dev:
                continue
            other_node = other_dev // gpus_per_node
            if node_id == other_node:
                bandwidth_matrix[dev_id][other_dev] = 1.0  # full intra-node BW
            else:
                bandwidth_matrix[dev_id][other_dev] = inter_ratio  # scaled inter-node BW

    interconnect = InterConnectModule(
        device_count=total_gpus,
        topology=topology,
        link_module=intra_link,
        link_count_per_device=intra_node_links_per_gpu,
        bandwidth_matrix=bandwidth_matrix,
    )
    # Store node mapping as attribute for use by hierarchical AllReduce
    interconnect.device_node_mapping = device_node_mapping
    interconnect.num_nodes = num_nodes
    interconnect.gpus_per_node = gpus_per_node
    interconnect.inter_node_link = inter_link
    interconnect.inter_node_links_per_gpu = inter_node_links_per_gpu
