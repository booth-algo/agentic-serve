...
from llmcompass.software_model.silu import SiLU


from llmcompass.software_model.utils import Tensor, DataType
from llmcompass.software_model.communication_primitives import AllReduceMultiPCB, AllReduceHierarchical
...
def _get_kernel_predictor(profiles_dir: str = None):
    ...
    if profiles_dir is None:
        ...
        profiles_dir = os.path.join(base, 'profiler', 'profiles', 'H100')
...
def _load_dense_calibration(profiles_dir: str = None) -> dict:
    ...
    if profiles_dir is None:
        base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        profiles_dir = os.path.join(base, 'profiler', 'profiles', 'A100')
...
def _get_perop_predictor():
    global _perop_predictor_cache
    if _perop_predictor_cache is not None:
        return _perop_predictor_cache
    import pickle, os
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    pkl_path = os.path.join(base, 'profiler', 'profiles', 'A100', 'perop_analytical_v4.pkl')
...
def _compute_perop_features(n_tokens, op, d_model, n_heads, n_kv_heads, intermediate_size, num_experts, top_k, batch_size=1, seq_len=None, kv_cache_len=0):
...
def _get_bsseq_predictor():
    ...
    import pickle, os
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    pkl_path = os.path.join(base, "profiler", "profiles", "A100", "layer_predictor_bsseq.pkl")
    if not os.path.isfile(pkl_path):
        return None
    with open(pkl_path, "rb") as f:
        data = pickle.load(f)
...
class TransformerBlockInitComputationTP(Operator):
    ...
    def compile_and_simulate(self, system: System, compile_mode: str):
        ...
        perop_model = _get_perop_predictor()
        if perop_model is not None and self.use_ml_predictor:
            ...
            for op in ["attn", "ffn", "norm_pre", "norm_post"]:
                features = _compute_perop_features(
            ...
            if dev_cnt > 1:
                ...
                ar_lat = _calibrated_allreduce_latency(ar_bytes)
                if ar_lat is not None:
                    exposed_ar = ar_lat * 2 * (1.0 - 0.95)  # 2 allreduces, 95% overlap
                    lat_sum += exposed_ar * 1e6  # seconds -> us
            lat_sum = _post_prediction_correction(lat_sum, n_tokens, d, getattr(self, 'num_experts', 0), b, s)
...
def _calibrated_allreduce_latency(size_bytes):
    cal = _get_allreduce_calibration()
    if cal:
