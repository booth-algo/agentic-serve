#!/usr/bin/env bash
# =============================================================================
# SGLang High-Concurrency Single-Turn — conc 200/256/320
# Only runs small models (8B, 9B) where high conc is feasible.
# 70B/72B/27B are skipped (memory-constrained, max conc 80).
# =============================================================================
set -uo pipefail
...
WARMUP=5
TIMEOUT=300
ST_PROFILES="chat-singleturn coding-singleturn"
CONC_SWEEP="200 256 320"
MAX_SERVER_WAIT=600

source "$REPO_ROOT/scripts/gpu_scheduler.sh"

log() { echo -e "\033[0;32m[SGHC]\033[0m $1"; }

# Only small models — 70B/72B/27B are memory-constrained at conc 80 max
declare -a MODELS=(
...
log "║  4 models × 2 profiles × 3 conc = 24 runs               ║"
log "╚══════════════════════════════════════════════════════════╝"
