# llm_predict_legacy — legacy predictor library for AgentServe-Bench
...
`llm_predict_legacy` is the legacy LLMCompass-derived stack for per-kernel,
per-op, and end-to-end latency prediction for
...
The current cache-aware serving predictor package is `llm_predict/`.
...
```python
from llm_predict_legacy.predictors.dispatch import PredictorDispatch
...
aws --profile r2 s3 sync \
  ...
  llm_predict_legacy/profiling/data/A100/ \
