...
class Reshape(Operator):
    ...
    def __call__(self, input: Tensor, output_shape: List[int]) -> Tensor:
        output_size = size(output_shape)
        # Allow flexible reshaping for performance modeling:
        # - output_size <= input.size: model only utilized portion (e.g., uneven head assignment)
        # - output_size >= input.size: model with implicit zero padding (remainder dimension handling)
        # This supports uneven tensor parallelism where dimensions don't divide evenly
        # For performance modeling, we model the bottleneck (fully utilized case)
        # Original strict check was: assert input.size == output_size
        # Now relaxed for performance modeling flexibility
        self.flop_count = 0
        self.load_count = 0
        self.store_count = 0
        self.io_count = 0
