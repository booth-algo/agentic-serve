from llm_predict_legacy.utils import size
from llm_predict_legacy.models.hardware.device import Device
from llm_predict_legacy.models.software.operators import Operator
from llm_predict_legacy.models.software.utils import Tensor, DataType
...
class BatchedMatmul(Operator):
    ...
    def roofline_model(self, pcb_module: Device):
        if not hasattr(self, 'M') or not hasattr(self, 'K') or not hasattr(self, 'N') or not hasattr(self, 'bs'):
            raise AttributeError(
                "BatchedMatmul.roofline_model() requires __call__() to be invoked first "
                "to set M, K, N, and bs attributes. Please call the operator with input tensors first."
            )
    ...
    def simulate(
        ...
        if self.look_up_table is None:
            self.look_up_table = pd.read_csv(
                os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'systolic_array_model', f'look_up_table_{pcb_module.compute_module.core.systolic_array.array_height}_{pcb_module.compute_module.core.systolic_array.array_width}.csv'),
                header=None,
                names=[
                    "M",
                    "N",
    ...
    def simulate_systolic_array_cycle_count(
        ...
        try:
            ...
            try:
                ...
                temp_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'systolic_array_model', 'temp')
                os.makedirs(temp_dir, exist_ok=True)
                config = os.path.join(temp_dir, f'systolic_array_{os.getpid()}.cfg')
                with open(config, "w") as f:
                    ...
                    # scalesim requires these buffer options
                    f.writelines("ReadRequestBuffer: 60\n")
                    f.writelines("WriteRequestBuffer: 60\n\n")
                    f.writelines("[layout]\n")
                    f.writelines("IfmapCustomLayout: 0\n")
                    f.writelines("FilterCustomLayout: 0\n")
                    ...
                    # scalesim expects this option; without it, configparser raises
                    # NoOptionError('UseRamulatorTrace', 'run_presets')
                    f.writelines("UseRamulatorTrace: 0\n")

                topology = os.path.join(temp_dir, f'matmul_{os.getpid()}.csv')
                with open(topology, "w") as f:
                    f.writelines("Layer, M, N, K\n")
                    f.writelines(f"matmul1, {M}, {N}, {K},\n")

                # Create an empty layout file (required by scalesim even when custom layouts are disabled)
                layout = os.path.join(temp_dir, f'layout_{os.getpid()}.csv')
                with open(layout, "w") as f:
                    pass  # Empty file is valid when custom layouts are disabled

                logpath = temp_dir + os.sep
                s = scalesim(
                ...
                lookup_table_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'systolic_array_model')
                os.makedirs(lookup_table_dir, exist_ok=True)
                lookup_table_file = os.path.join(lookup_table_dir, f'look_up_table_{array_height}_{array_width}.csv')
                with open(
                    lookup_table_file,
                    "a",
                ) as f:
