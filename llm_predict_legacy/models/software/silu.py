...
# SiLU(x) = x * sigmoid(x) — 2 FLOPs per element (vs GeLU's ~8+exp)
class SiLU(Operator):
    ...
    def roofline_model(self, pcb_module: Device):
        if not hasattr(self, 'computational_graph') or not hasattr(self, 'M'):
            raise AttributeError(
                "SiLU.roofline_model() requires __call__() to be invoked first "
                "to set computational_graph and M attributes. Please call the operator with input tensor first."
            )
        ...
        # SiLU = x * sigmoid(x): 2 FLOPs per element
        total_flop_count = M * 2
        compute_latency = (
            total_flop_count
