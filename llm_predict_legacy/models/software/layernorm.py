...
class LayerNorm(Operator):
    ...
    def roofline_model(self, pcb_module: Device):
        if not hasattr(self, 'M') or not hasattr(self, 'N'):
            raise AttributeError(
                "LayerNorm.roofline_model() requires __call__() to be invoked first "
                "to set M and N attributes. Please call the operator with input tensor first."
            )
