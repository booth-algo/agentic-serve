...
def main() -> None:
    ...
    if pkg_dir.name != "per_kernel":
        pkg_dir = Path(os.environ.get("REPO_ROOT", ".")) / "llm_predict_legacy" / "training" / "per_kernel"
    repo_root = pkg_dir.parent.parent.parent

    data_path = Path(args.data) if args.data else repo_root / "llm_predict_legacy" / "training" / "per_op" / "data" / "per_op_labeled.csv"
    ...
    if args.out_dir:
        ...
        out_dir = repo_root / "llm_predict_legacy" / "profiling" / "data" / args.gpu / "trained" / "per_kernel"
