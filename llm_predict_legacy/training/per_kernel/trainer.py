...
- Outer loop over GPUs (A100 / RTX3090 / RTX2080Ti); one pkl per
  (GPU × family) lands at `llm_predict_legacy/profiling/data/{gpu}/trained/per_kernel/`.
- Held-out model list driven by `model_specs.HELD_OUT_BY_GPU`.
- Feature columns match `llm_predict_legacy/predictors/per_kernel/predictor.py` so the
...
from llm_predict_legacy.training.per_kernel.ensure_data import ensure_kernels_csv
...
def main() -> None:
    ...
    llm_predict_legacy_dir = pkg_dir.parents[1]   # llm_predict_legacy/ — matches PerKernelPredictor._pkl_dir()
    data_csv = Path(args.data) if args.data else pkg_dir / "data" / "kernels_labeled.csv"
    data_csv = ensure_kernels_csv(data_csv)
    out_dir  = Path(args.out_dir) if args.out_dir else llm_predict_legacy_dir / "profiling" / "data"
