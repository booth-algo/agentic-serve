...
Usage:
    python -m llm_predict_legacy.training.per_kernel.scripts.build_profiling_manifest \
        ...
        --r2-profile r2 \
        --out llm_predict_legacy/training/per_kernel/profiling_manifest.yaml
...
def trained_pkl_status(
    ...
    trained_dir = repo_root / "llm_predict_legacy" / "profiling" / "data" / gpu / "trained"
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--out", default="llm_predict_legacy/training/per_kernel/profiling_manifest.yaml")
    args = parser.parse_args(argv)

    ncu_root = Path(args.ncu_root)
    out_path = Path(args.out)
    # repo_root: scripts/ -> per_kernel/ -> training/ -> llm_predict_legacy/ -> repo/
