...
Offline training pipeline for the per-kernel shape-only XGBoost predictors consumed by `llm_predict_legacy/predictors/per_kernel/predictor.py`.

Ported from `runpod:~/per-kernel-rebuild/` (baseline: commit 9483ba9 scrap of per-category RF; the composition method — enumerate ops → predict per op → sum — survived and lives in `llm_predict_legacy/predictors/per_kernel/predictor.py`).
...
| `model_specs.py` | Legacy `ModelConfig` dataclass with embedded architecture specs | new |
...
- Model archs: embedded in `model_specs.py`.

## Output artifacts

- `llm_predict_legacy/training/per_kernel/data/kernels_labeled.csv` — single combined labeled dataset with `gpu` column.
- `llm_predict_legacy/profiling/data/{A100,RTX3090,RTX2080Ti}/trained/per_kernel/perkernel_{gemm,flash_attn,elementwise,misc}_shape_v2.pkl` — 12 pkls (or fewer if 2080Ti misc has <5 rows).
- `llm_predict_legacy/training/per_kernel/reports/{gpu}_validation.md` — per-GPU MAPE + aggregate error tables.
...
python -m llm_predict_legacy.training.per_kernel.labeler \
    --ncu-root /root/llm/profiling-data \
    --out llm_predict_legacy/training/per_kernel/data/kernels_labeled.csv

# Phase A2 + A3 — train + validate
python -m llm_predict_legacy.training.per_kernel.trainer \
    --data llm_predict_legacy/training/per_kernel/data/kernels_labeled.csv \
    --out-dir llm_predict_legacy/profiling/data

# Phase 4 — composition validation
python -m llm_predict_legacy.training.per_kernel.validate --gpu A100
python -m llm_predict_legacy.training.per_kernel.validate --gpu RTX3090
python -m llm_predict_legacy.training.per_kernel.validate --gpu RTX2080Ti
```
...
for GPU in A100 RTX3090 RTX2080Ti; do
  python -m llm_predict_legacy.training.per_kernel.roofline_labeler \
      --sweep-prefix /tmp/ncu_gemm_sweep_${GPU} \
      --gpu ${GPU} \
      --append-to llm_predict_legacy/training/per_kernel/data/kernels_labeled.csv
done

# 4) Retrain + re-validate (pkls overwrite in place).
python -m llm_predict_legacy.training.per_kernel.trainer
python -m llm_predict_legacy.training.per_kernel.validate
```
