...
`llm_predict_legacy/predictors/per_op/predictor.py`.

Productised from the historical ad-hoc per-op training script —
same 26-feature analytical formulation, same XGB regression on
`log(duration_us)`, but structured to mirror
`llm_predict_legacy/training/per_kernel/` so the two training pipelines share
...
`llm_predict_legacy/training/per_kernel/model_specs._DIR_TO_SHORT` — the same
...
- `llm_predict_legacy/training/per_op/data/per_op_labeled.csv` — the combined
  labeled dataset (schema: see `labeler.OUT_COLS`).
- `llm_predict_legacy/profiling/data/{A100,RTX3090,RTX2080Ti}/trained/per_op/perop_v5_shape.pkl`
  ...
  `llm_predict_legacy/predictors/per_op/predictor.py` still reads.
- `llm_predict_legacy/training/per_op/reports/{labeling,training}_report.{md,json}`
...
```bash
# Phase A1 — label
python -m llm_predict_legacy.training.per_op.labeler \
    --raw-root /path/to/profiling_data \
    --out llm_predict_legacy/training/per_op/data/per_op_labeled.csv

# Phase A2 + A3 — train
python -m llm_predict_legacy.training.per_op.trainer \
    --data llm_predict_legacy/training/per_op/data/per_op_labeled.csv \
    --out-dir llm_predict_legacy/profiling/data

# Smoke tests
python -m pytest llm_predict_legacy/training/per_op/tests/ -v
```
...
1. `llm_predict_legacy/training/per_op/feature_spec.py:compute_features` (this module — canonical).
2. `llm_predict_legacy/predictors/per_op/features.py:compute_perop_features` (runtime).
3. `llm_predict_legacy/models/software/transformer.py:175:compute_perop_features` (duplicated).
...
4. Refactor `predictors/per_op/features.py` and `models/software/transformer.py`
   to `from llm_predict_legacy.training.per_op.feature_spec import compute_features`.
