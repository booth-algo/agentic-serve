...
1. the historical per-op training prototype's transformer feature logic
   (the legacy ad-hoc training script).
2. `llm_predict_legacy/predictors/per_op/features.py:compute_perop_features()`
...
`llm_predict_legacy/models/software/transformer.py` — it is how a labeler
...
#   - llm_predict_legacy/models/software/transformer.py (the layer walker)
#   - llm_predict_legacy/predictors/per_op/features.py (runtime)
OP_MAP: dict[str, str] = {
    "self_attn":                 "attn",
    "mlp":                       "ffn",
