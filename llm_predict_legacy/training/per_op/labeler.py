...
the historical per-op training prototype's phase-1 loader.

Raw input schema
----------------
Each input CSV is produced by torch.profiler CUDA-event capture
(e.g. the historical per-op profiler). Expected columns (either spelling
is accepted):
...
from llm_predict_legacy.training.per_kernel import model_specs
