...
step_overhead_per_req_us: per-active-request overhead (block-table indirection, KV mgmt).
    Physics-based defaults (~200us scheduler + ~10us per request).
    Calibration captured kernel prediction error, not true scheduler overhead (r² < 0.1).
"""
...
GPU_SPECS: dict[str, GpuSpec] = {
    "H100": GpuSpec(
        ...
        step_overhead_base_us=200.0,       # scheduler loop + CUDA graph replay
        step_overhead_per_req_us=10.0,     # block-table indirection per active request
    ),
    "A100": GpuSpec(
        name="A100-SXM4-40GB",
