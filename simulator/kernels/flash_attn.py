...
# The H100 residual model is trained from PyTorch SDPA NCU data. For decode
# extrapolation, cap it so those measurements do not dominate vLLM serving TPOT.
_DECODE_RESIDUAL_MAX_MULTIPLIER = 2.0
        ...
        if table_path.exists():
            ...
            with open(table_path) as f:
                ...
                for row in reader:
                    ...
                    q = int(row.get("q_len", row.get("seq_len", 0)))
                    key = (q, int(row["kv_len"]),
    ...
    def predict(self, seq_len: int, n_heads: int, head_dim: int,
        ...
        if self._xgb is not None:
            ...
            if phase == "decode" or seq_len == 1:
                return min(
                    predicted,
                    baseline * _DECODE_RESIDUAL_MAX_MULTIPLIER,
                )
            return predicted
