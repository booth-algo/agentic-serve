...
_PROFILE_LOOKUP_ALIASES = {
    "coding-singleturn": ("coding-singleturn", "coding-agent"),
    "coding-agent": ("coding-agent", "coding-singleturn"),
}


def _profile_lookup_keys(profile: str | None) -> tuple[str, ...]:
    if profile is None:
        return ()
    return _PROFILE_LOOKUP_ALIASES.get(profile, (profile,))
...
def _profile_factor_table_from_artifact(item: dict | None, field: str,
    ...
    raw = item.get(field, {}) if item else {}
    profile_block = None
    for profile_key in _profile_lookup_keys(profile):
        candidate = raw.get(profile_key)
...
def moe_decode_correction_factor(gpu: str, backend: str,
    ...
    if profile is not None:
        ...
        if isinstance(model_profiles, dict):
            profile_block = None
            for profile_key in _profile_lookup_keys(profile):
                candidate = model_profiles.get(profile_key)
                if isinstance(candidate, dict):
                    profile_block = candidate
                    break
