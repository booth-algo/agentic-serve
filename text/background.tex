...
Agrawal et al.'s Sarathi-Serve~\citep{sarathi} introduces chunked prefills and stall-free scheduling, achieving up to 2.6$\times$ higher serving capacity on Llama-13B and 3.7$\times$ on Yi-34B compared to Orca-style batching, but does not benchmark agentic workloads or provide kernel-level profiling.
Patel et al.'s Splitwise~\citep{splitwise} disaggregates inference phases at the cluster level, delivering 1.4$\times$ higher throughput at 20\% lower cost, but evaluates only single-turn chat traces.
Cho et al.'s LLMServingSim~v2~\citep{llmservingsim} achieves 1.9\% error for GPU-based LLM serving with runtime-loop simulation and supports MoE, but uses lookup tables rather than generalizable ML predictors.
...
\textbf{Feature} & \textbf{LLMCompass} & \textbf{Vidur} & \textbf{LLMServSim\,2} & \textbf{DistServe} & \textbf{MLPerf} & \textbf{ML.ENERGY} & \textbf{Ours} \\
 & \scriptsize\citep{llmcompass} & \scriptsize\citep{vidur} & \scriptsize\citep{llmservingsim} & \scriptsize\citep{distserve} & \scriptsize\citep{mlperf} & \scriptsize\citep{mlenergy} & \\
\midrule
...
\quad Reported latency error                  & 4.1\% & ${<}$9\% & 1.9\% & ${<}$2\% & --- & --- & 4.1\% \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
