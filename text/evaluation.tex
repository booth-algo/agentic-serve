...
Table~\ref{tab:single-turn} reports TTFT, TPOT, and request throughput at saturation for six models under Tier~2 chat and Tier~1 agentic workloads.

Llama-3.1-8B serves chat-short at 282\,ms TTFT and 55.15\,req/s; under coding-agent the same model reaches 598\,ms TTFT and 15.03\,req/s, a 3.7$\times$ reduction in request throughput. Qwen3.5-9B shows a similar pattern: chat-short at 533\,ms and 23.78\,req/s, coding-agent at 1{,}583\,ms and 14.39\,req/s. Qwen3.5-27B reaches 2{,}926\,ms TTFT under coding-agent, with req/s dropping from 16.45 to 7.23. Mixtral-8$\times$7B serves chat-short at 400\,ms and 35.33\,req/s but lacks coding-agent data.

(no gpt oss models) MoE models show a different throughput pattern. gpt-oss-20b drops from 46.58 to 41.10\,req/s under coding-agent (1.1$\times$), while gpt-oss-120b drops from 33.67 to 21.48\,req/s (1.6$\times$). gpt-oss-20b shows the smallest TTFT increase (1{,}247\,ms to 2{,}272\,ms, 1.8$\times$); gpt-oss-120b the largest (519\,ms to 2{,}905\,ms, 5.6$\times$).

TPOT at saturation does not consistently increase from chat to coding-agent. Llama-3.1-8B rises from 15.2 to 56.9\,ms. Qwen3.5-9B drops from 55.3 to 26.0\,ms; the two profiles saturate at different concurrency levels, producing different batch sizes and scheduling dynamics at the measurement point.
...
Figure~\ref{fig:quadrant} shows the system-level view across 1{,}466 benchmark configurations. Tier~1 agentic workloads (red, $n$\,=\,215) cluster at median OI\,=\,4{,}774; Tier~2 chat workloads (blue, $n$\,=\,758) cluster at median OI\,=\,85. The two tiers are separated by 56$\times$ in median operational intensity, with no overlap in their interquartile ranges.
