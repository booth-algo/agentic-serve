...
Table~\ref{tab:single-turn} reports TTFT, TPOT, and request throughput at saturation for six models under Tier~2 chat and Tier~1 agentic workloads. Llama-3.1-8B serves chat-short at 282\,ms TTFT and 55.15\,req/s; under coding-agent the same model reaches 598\,ms TTFT and 15.03\,req/s. Qwen3.5-9B and Qwen3.5-27B show the same pattern at larger scale: 533\,ms and 876\,ms for chat, rising to 1{,}583\,ms and 2{,}926\,ms for coding-agent. (no gpt oss models) gpt-oss-20b shows the smallest TTFT increase (1{,}247\,ms to 2{,}272\,ms, 1.8$\times$); gpt-oss-120b shows the largest (519\,ms to 2{,}905\,ms, 5.6$\times$). TPOT at saturation ranges from 15 to 109\,ms across models. Within a given model, TPOT does not consistently increase from chat to coding-agent: Llama-3.1-8B rises from 15.2 to 56.9\,ms, while Qwen3.5-9B drops from 55.3 to 26.0\,ms.
...
Table~\ref{tab:concurrency-sweep} reports latency scaling under concurrency for Llama-3.1-8B on H100. Chat-short TTFT grows from 27.5\,ms at concurrency 1 to 281.8\,ms at concurrency 320. Coding-agent TTFT starts higher at 74.4\,ms and reaches 823\,ms at concurrency 120, with p99 exceeding 4{,}750\,ms at concurrency 320. TPOT at concurrency 1 is 6.2\,ms for chat-short and 6.5\,ms for coding-agent. At concurrency 320, chat-short TPOT reaches 15.2\,ms while coding-agent reaches 56.9\,ms. Request throughput saturates at 55.15\,req/s for chat-short and 15.03\,req/s for coding-agent. Coding-agent TTFT p50 at concurrency 320 (597.8\,ms) is lower than at concurrency 120 (823.1\,ms); this non-monotonicity reflects variance in the single-trial measurement at high concurrency, where scheduling order and prefill chunking interact with the request mix.
...
Table~\ref{tab:multi-turn} reports multi-turn serving performance for three dense models under Tier~2 chat and Tier~1 agentic sessions. Llama-3.1-8B serves chat multi-turn at 498\,ms TTFT and 19.37\,req/s; under SWE-Bench multi-turn the same model reaches 2{,}375\,ms TTFT and 4.74\,req/s, a 4.1$\times$ reduction in request throughput. TPOT increases from 12.1\,ms to 136.0\,ms between chat and SWE-Bench multi-turn for Llama-3.1-8B. Qwen3.5-9B and Qwen3.5-27B show smaller TPOT increases (19.0 to 39.9\,ms and 36.2 to 61.7\,ms respectively). Qwen3.5-27B is an exception: its swebench-mt TTFT (1{,}084\,ms) is lower than chat-mt-short (1{,}705\,ms), and its swebench-mt req/s (9.28) is higher than chat-mt-short (7.39).


\subsection{Kernel-Level Roofline Analysis}
\label{sec:kernel_roofline}

Table~\ref{tab:kernel-roofline} reports per-kernel time and arithmetic intensity for two-layer forward passes of Llama-3.1-8B and gpt-oss-20b at batch size 1 and sequence length 512. Prefill GEMMs in Llama-8B reach AI\,=\,1{,}764 and achieve 785\,TFLOPS, 6$\times$ above the H100 ridge point (AI\,=\,295). Decode GEMMs on the same weights sit at AI\,=\,4.0 and 2.9\,TFLOPS; every kernel category in decode is memory-bound. gpt-oss-20b executes 63\% more kernels per forward pass (83 vs 51), with elementwise operations accounting for 52.4\% of prefill time compared to 35.8\% in the dense model. Prefill operates in the compute-bound regime; decode operates in the memory-bound regime. Figure~\ref{fig:roofline} visualises this separation across all five profiled models. Figure~\ref{fig:quadrant} shows the system-level view: agentic workloads cluster in the compute-bound, high-capacity-footprint quadrant, while chat workloads cluster in the bandwidth-bound quadrant.
